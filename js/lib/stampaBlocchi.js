...
//  nel CSS).
//
//  Come per la resa in Word, qui non c'è niente di una sezione in
//  particolare: si ricevono i blocchi già costruiti e il titolo della
//  finestra. Assistenze e formazione stampano così lo stesso foglio.
// ============================================================
...
export async function htmlDocumento(blocchi, titolo) {
  const carta = await caricaCarta();
  return `<!doctype html><html lang="it"><head><meta charset="utf-8">
<title>${esc(titolo || 'Documento')}</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* table-layout:fixed è la ragione per cui le tabelle larghe non escono dal
     foglio: con la disposizione automatica, molte colonne chiedono più
     larghezza della pagina e in stampa vengono semplicemente tagliate, perché
     sulla carta non c'è nessuno scorrimento orizzontale. Così invece le
     colonne si comprimono e il testo va a capo. */
  ...
  /* Con molte colonne rimpicciolire il corpo costa meno che vedere
     "16/11/20 26" spezzato in due righe. */
  table.dati.compatta { font-size: 8.5pt; }
  table.dati.compatta th, table.dati.compatta td { padding: 1.2mm 1.4mm; }
  /* Le intestazioni sono spesso i testi più lunghi della tabella: un corpo
     più piccolo solo per loro evita che vengano spezzate a metà parola,
     senza toccare la leggibilità dei dati. */
...
export async function anteprimaDocumento(blocchi, titolo) {
  apriFinestra(await htmlDocumento(blocchi, titolo));
}
...
export async function stampaDocumento(blocchi, titolo) {
  const finestra = apriFinestra(await htmlDocumento(blocchi, titolo));
