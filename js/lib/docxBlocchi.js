// ============================================================
//  DAI BLOCCHI AL .docx SULLA CARTA INTESTATA
...
//  corpo con i blocchi del documento. Così il risultato è identico a un
...
//
//  Qui dentro non c'è niente di una sezione in particolare: si ricevono i
//  blocchi già costruiti (vedi il documento.js di ciascuna sezione) e il nome
//  del file. Assistenze e formazione producono così due Word identici nella
//  forma, che è quello che ci si aspetta da documenti dello stesso Comitato.
//
...
export async function generaDocx(blocchi, nome) {
  ...
  const voci = Object.entries(carta.file).map(([nomeVoce, dati]) => {
    if (nomeVoce === 'word/document.xml') return { nome: nomeVoce, dati: byte(documento) };
    if (nomeVoce === '[Content_Types].xml') return { nome: nomeVoce, dati: byte(contentTypes) };
    return { nome: nomeVoce, dati };
  });

  return { blob: new Blob([creaZip(voci)], { type: MIME_DOCX }), nome };
}

// Scarica il documento generato.
export async function scaricaDocx(blocchi, nomeDelFile) {
  const { blob, nome } = await generaDocx(blocchi, nomeDelFile);
