...
//  correggere un indirizzo, aggiungere il telefono del referente, togliere
//  un'azienda che non c'è più.
//
...
export async function renderRubrica(view, ctx) {
  ...
  view.appendChild(el(`<div class="page-head">
    <div><h1>Rubrica committenti</h1><p>Aziende ed enti che ci chiedono i corsi</p></div>
    <button class="btn primary" id="nuovo">➕ Nuovo committente</button>
  </div>`));

  const toolbar = el(`<div class="toolbar">
    <div class="search"><span class="search-icon" aria-hidden="true">🔍</span>
      <input type="text" id="q" placeholder="Cerca per nome, partita IVA o referente…"></div>
  </div>`);
  view.appendChild(toolbar);

  const card = el(`<div class="card"><div class="tbl-wrap"><table class="tbl">
    <thead><tr><th>Azienda / ente</th><th>Codice fiscale / P.IVA</th><th>Indirizzo</th><th>Referente</th><th></th></tr></thead>
  ...
  function disegna() {
    ...
    if (!righe.length) {
      tbody.appendChild(el(`<tr><td colspan="5" class="muted" style="text-align:center;padding:26px">
        ${elenco.length ? 'Nessun risultato' : 'Rubrica vuota: le aziende si aggiungono da qui o dal preventivo, con «Salva in rubrica».'}</td></tr>`));
      return;
    }
    ...
    for (const c of righe) {
      ...
      tr.querySelector('[data-del]').addEventListener('click', async () => {
        ...
        try {
          ...
          toast('Committente eliminato', 'ok');
          disegna();
  ...
  // La scheda si apre in un riquadro sopra l'elenco: si aggiunge un'azienda
  // senza perdere di vista quelle che ci sono già (utile per accorgersi che
  // c'è di già, scritta in un altro modo).
...
export function schedaCliente(cliente, onSalvato) {
  ...
  const corpo = el(`<div>
    <div class="field"><label>Azienda / ente *</label>
      <input type="text" id="r-nome" value="${esc(c.nome || '')}" placeholder="es. Omnia Service s.r.l."></div>
    <div class="form-row">
  ...
  // è un committente nuovo, non una modifica. A dirlo è l'id, non il fatto che
  // i campi siano pieni.
  const esistente = !!c.id;
  const { close } = openModal({ title: esistente ? 'Modifica committente' : 'Nuovo committente', body: corpo, footer: piede });
  ...
  piede.querySelector('[data-ok]').addEventListener('click', async () => {
    ...
    if (!dati.nome) { toast('Manca il nome dell\'azienda', 'err'); corpo.querySelector('#r-nome').focus(); return; }
    btn.disabled = true; btn.innerHTML = '<span class="spinner sm"></span> Salvo…';
    try {
      const salvato = await clienti.save(dati);
      toast(esistente ? 'Committente aggiornato' : 'Committente aggiunto in rubrica', 'ok');
      close();
      onSalvato?.(salvato);
    } catch (e) {
      toast(e.message, 'err');
